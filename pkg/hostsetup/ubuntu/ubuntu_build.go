...
func downloadDep(c *dagger.Container, name string, dep hostsetup.Dep) *dagger.Container {
	if dep.Version != "" {
		name = fmt.Sprintf("%s=$(apt-cache madison %s | grep %q | awk '{print $3}' | head -n 1)", name, name, dep.Version)
	}
	...
	return c.
		WithExec([]string{
			"sh", "-c",
			fmt.Sprintf(`
apt-get install -y --download-only -o Dir::Cache::archives="/opt/offline-repo/debs" %s
`, name),
		})
}
